...
class CustomerListRepository(private val customerDao: CustomerDao, private val service: CustomerListService, private val preferences: SharedPreferenceWrapper, private val connectionChecker: ConnectionChecker) {

    var localStorageCustomers: List<Customer> = emptyList()

    fun addCustomer(customer: Customer): Observable<Customer> = service.addCustomer(preferences.baseUrl + ADD_CUSTOMER_ENDPOINT, customer).map { it.data }
    ...
    private fun getCustomersFromApi() = service.getCustomerList(preferences.baseUrl + CUSTOMERS_ENDPOINT).map { it.data }

    private fun getCustomersFromLocalStorage() = customerDao.getCustomerList().doOnSuccess { localStorageCustomers = it }
    ...
    private class DeleteCustomerAsyncTask internal constructor(private val mAsyncTaskDao: CustomerDao, private val callback: (() -> Unit)? = null) : AsyncTask<Customer, Void, Void>() {
