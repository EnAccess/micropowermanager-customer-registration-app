...
class AddCustomerRepository(
    ...
    fun getManufacturers() =
        service.getManufacturers(preferences.baseUrl + MANUFACTURERS_ENDPOINT).map { it.data }

    fun getMeterTypes() =
        service.getMeterTypes(preferences.baseUrl + METER_TYPES_ENDPOINT).map { it.data }
    ...
    fun getConnectionGroups() =
        service.getConnectionGroups(preferences.baseUrl + CONNECTION_GROUPS_ENDPOINT)
            .map { it.data }

    fun getConnectionTypes() =
        service.getConnectionTypes(preferences.baseUrl + CONNECTION_TYPES_ENDPOINT).map { it.data }

    fun getSubConnectionTypes() =
        service.getSubConnectionTypes(preferences.baseUrl + SUB_CONNECTION_TYPES_ENDPOINT)
            .map { it.data }

    private class AddCustomerAsyncTask internal constructor(private val mAsyncTaskDao: CustomerDao) :
