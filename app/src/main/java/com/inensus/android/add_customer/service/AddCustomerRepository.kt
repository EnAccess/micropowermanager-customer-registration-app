...
class AddCustomerRepository(private val customerDao: CustomerDao, private val service: AddCustomerService, private val preferences: SharedPreferenceWrapper) {
    ...
    fun getConnectionGroups() = service.getConnectionGroups(preferences.baseUrl + CONNECTION_GROUPS_ENDPOINT).map { it.data }

    fun getConnectionTypes() = service.getConnectionTypes(preferences.baseUrl + CONNECTION_TYPES_ENDPOINT).map { it.data }

    fun getSubConnectionTypes() = service.getSubConnectionTypes(preferences.baseUrl + SUB_CONNECTION_TYPES_ENDPOINT).map { it.data }
